        ...
        Button(
          action: {
            ...
            if let idx = self.tally.data.firstIndex(where: { $0.date == self.tallyDatum.date }) {
              ...
              let idx =
                self.tally.data.firstIndex(where: { $0.date < self.tallyDatum.date })
                ?? self.tally.data.count
          ...
          HStack(alignment: .firstTextBaseline) {
            ...
            if self.tally.kind == .completion {
              ...
              Button(action: {
                ...
                Image(
                  systemName: self.tallyDatum.boolValue
                    ? "checkmark.circle.fill" : "checkmark.circle"
                )
                .resizable()
                .frame(width: 20, height: 20)
                .foregroundColor(
                  self.tallyDatum.boolValue ? .customAccent : Color(UIColor.tertiaryLabel))
              }
              .padding(.bottom, -2)
            } else {
